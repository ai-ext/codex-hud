...
### 새로운 기능
- **스크롤 지원** — 콘텐츠가 터미널보다 클 때 마우스 휠, j/k, 화살표, PgUp/PgDn으로 스크롤 가능
- **스크롤 인디케이터** — 스크롤 가능 시 우하단에 `↕ XX%` 위치 표시

### 변경 사항
- bubbles viewport 컴포넌트 도입 (`github.com/charmbracelet/bubbles v1.0.0`)
- 노멀 모드에서 모든 섹션을 항상 렌더링 (뷰포트가 스크롤 처리)

## [0.4.1] - 2026-03-16

### 버그 수정
- **세션 전환 감지 개선 (/resume)** — `/resume` 등으로 새 세션 파일이 생성될 때 HUD가 자동 감지하도록 개선
- **fsnotify 실패 복원력** — fsnotify 초기화 실패 시에도 폴링이 계속 동작하도록 수정 (이전: 함수 즉시 종료)
- **partial line 데이터 유실 방지** — 개행 없이 끝나는 불완전한 줄이 버퍼링 대신 버려지던 문제 수정
- **이중 tailing 방지** — 비-fresh 모드에서 동일 파일을 두 번 감시하던 문제 수정

### 새로운 기능
- **디버그 모드** — `--debug` 플래그로 세션 감지 과정 상세 로깅 활성화
